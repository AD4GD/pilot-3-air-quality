...
def process_kriging(cdict,
    ...
    if no_filter:
        iotdata = combids['hour_median_corr']
    else:
        qaflag = combids['hour_median_corr_distflag'].isel(radius=1)
        iotdata = combids['hour_median_corr'].where(qaflag)
    ...
    for itime, timeval in enumerate(seliotdata.time.values):
        ...
        df = df.rename(columns={'hour_median_corr': 'measurement'})
...
def main(day, inputfn, outfn, area='CAMS', no_filter=False):
    """
    ...
    cdict = areadict[area]
    process_kriging(cdict, day, inputfn, outfn,
                    no_filter=no_filter,
                    tree_model='kdtree')
