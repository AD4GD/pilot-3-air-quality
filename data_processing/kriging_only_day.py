...
def process_kriging(cdict,
    ...
    ----------
    cdict : dict
        Dictionary with spatial domain information.
    month : datetime
        Month to process.
    iotmeasfn : Path
    ...
    no_filter : bool
        Whether to not filter outliers in the IoT data. Default is False.
    tree_model : str
    ...
    lsmfn = Path(scriptdir, 'input_for_correction', 'gmt_lsm.nc')
    lsmds = xr.open_dataset(lsmfn)
    ...
    measds = xr.open_dataset(iotmeasfn)
    ...
    if no_filter:
        iotdata = combids['day_corr']
    else:
        qaflag = combids['day_corr_distflag'].isel(radius=1)
        iotdata = combids['day_corr'].where(qaflag)

    # select data for kriging (mean over time)
    seliotdata = iotdata
    ...
    for itime, timeval in enumerate(seliotdata.time.values):
        ...
        df = df.rename(columns={'day_corr': 'measurement'})
    ...
    mergeds.to_netcdf(outfn, encoding=encodedict)


def main(month, inputfn, outfn, area='CAMS', no_filter=False):
    """
    Main function to perform kriging for a given day and area.
    ...
    outfn : Path
        Path to save results to.
    area : str
        Area to process. Options are 'CAMS' or 'CentralEurope'.
        Default is 'CAMS'.
    no_filter : bool
    ...
    areadict = {'CAMS': {'latmin': 35, 'latmax': 72,
                            'lonmin': -25, 'lonmax': 45,
                            'ds': 0.01},
    ...
    cdict = areadict[area]
    process_kriging(cdict, month, iotmeasfn, outfn,
                    no_filter=no_filter,
                    tree_model='kdtree')
