...
class Corrector:
    ...
    def __init__(self,
        ...
        self.xgbfold = Path(scriptdir, 'input_for_correction', 'hourly')

        self.xgbfn = Path(self.xgbfold, f'xgb_hour_{self.poll}.json')
        self.keepfn = Path(self.xgbfold, f'kept_clusters_hour_{self.poll}.csv')
        self.labfn = Path(self.xgbfold, f'iot_labels_{self.poll}.csv')
    ...
    def correct_sensor_community(self,
        ...
        """
        Function to correct Sensor.Community data using XGBoost model
        and ERA5 data.
        ...
        thres_hour_obs : int, optional
            Minimum number of hourly observations for correction.
            The default is 20.
        max_thres : float, optional
            Maximum threshold for pollutant values.
        ...
        # load XGBoost model and read input data of XGBoost training
        # to get unique cluster categories
        ...
        for idx in keepdf.index:
            xgblat = centerdf['Latitude'].loc[idx]
            xgblon = centerdf['Longitude'].loc[idx]
        ...
        iot_rh = thermo.relative_humidity_from_dewpoint(interds['iot_t2m'], interds['iot_d2m'])
        interds['iot_rh'] = iot_rh

        # add day of year to features and expand it to size of other features
        interds['doy'] = (('time', 'location'),
                          np.tile(interds.time.dt.dayofyear,
                          (len(interds.location), 1)).T)
        ...
        # add cosine and sine for hour of day
        interds['cos_hour'] = np.cos(2*np.pi*interds.time.dt.hour / 24)
        interds['sin_hour'] = np.sin(2*np.pi*interds.time.dt.hour / 24)

        # prefilter IoT data
        static_vars = ['lat', 'lon', 'cluster']
        dynamic_vars = [v for v in scomds.data_vars if v not in static_vars]
        iotds = scomds[dynamic_vars].where(scomds['num_obs'] > thres_hour_obs, drop=True)
        iotds = iotds.where(iotds[self.iotvar] < max_thres, drop=True)
        iotds = iotds.where(iotds[self.iotvar] > 1, drop=True)

        # add static variables to IoT dataset
        iotds['lat'] = scomds['lat'].sel(location=iotds.location)
        iotds['lon'] = scomds['lon'].sel(location=iotds.location)
        iotds['cluster'] = scomds['cluster'].sel(location=iotds.location)

        # collect data in dict of dataframes for output
        outdict = {k: {} for k in ['hour_median_corr', 'ens_bool', 'ens_std']}

        # check if IoT station is in cluster
        loc_cluster = iotds['location'][iotds['cluster'].isin(xgb_labels)]
        no_loc_cluster = iotds['location'][~iotds['cluster'].isin(xgb_labels)]

        print('Correcting data for clustered IoT stations...')
        for location in tqdm(loc_cluster.values, disable=False):
            lociot = iotds.sel(location=location)
            ...
            ens_std = pd.Series(np.full(ens_mean.shape, np.nan),
                                index=ens_mean.index)
            ens_bool = False

            # add corrected data to output dictionary
            outdict['hour_median_corr'][location] = ens_mean
            outdict['ens_bool'][location] = ens_bool
            outdict['ens_std'][location] = ens_std.astype(np.float32)

        daycorr = pd.DataFrame(outdict['hour_median_corr']).T
        ...
        nlc_iot = iotds.sel(location=no_loc_cluster)
        ...
        nlc_era.index = nlc_era.index.swaplevel('time', 'location')
        ...
        conds['distance'] = xr.DataArray(filldistance,
                                         dims=['cluster', 'time', 'location'])
        ...
        iotds['hour_median_corr'] = corrected_data
        iotds['ens_std'] = corrected_ens_std
        iotds['ens_bool'] = corrected_ens_bool

        # convert dtypes
        iotds['hour_median_corr'] = iotds['hour_median_corr'].astype(np.float32)
        iotds['ens_bool'] = iotds['ens_bool'].astype(bool)
        iotds['cluster'] = iotds['cluster'].astype('int32')
        iotds['cluster'].attrs['xgb_labels'] = xgb_labels
        iotds['cluster'].attrs['cluster_labels'] = unique_labels
    ...
    def outlier_filtering(self,
        ...
        interpol = hour_polds.interp(latitude=combids.lat,
                                     longitude=combids.lon,
                                     method='nearest')

        # now calculate alpha values at each IoT location
        alpha = np.log(combids['hour_median_corr'] / interpol)
        ...
        for statidx, (qdist, areaidx) in tqdm(enumerate(zip(querydist, queryidx)),
            ...
            loc_pol = combids['hour_median_corr'].isel(location=statidx)
    ...
    def run(self):
        ...
        corr_scomds = self.correct_sensor_community(scomds,
                                                    erads,
                                                    thres_hour_obs,
                                                    max_thres)

        # rename location names
        corr_scomds['location'] = [f'SC{loc}' for loc in corr_scomds.location.values]
        combids = corr_scomds

        # Filter for outliers
        print('Filtering for outliers...')
        boolda = self.outlier_filtering(combids, hour_polds)
        combids['hour_median_corr_distflag'] = boolda
