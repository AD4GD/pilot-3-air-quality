...
from utils import shiftgrid, latlon_to_geocentric
import variogram
...
class KrigingCAMS:
    """
    Class to perform penalized universal kriging of IoT data with (CAMS) model
    data as drift.
    ...
    def run(self, polds: xr.DataArray):
        """
        Main function to perform regridding of IoT data to given regular
        lat-lon grid using universal kriging on global and local scale
        with given model data as drift.

        Parameters
        ----------
        polds : xr.DataArray
            Model data to use as drift for kriging.
            Should be a DataArray with dimensions 'latitude', 'longitude',
            and 'time'.
        ...
        for itime, timeval in enumerate(seliotdata.time.values):
            ...
            # select CAMS data for this time
            modtime = pd.to_datetime(timeda.time.values)
            modpol = polds.sel(time=modtime)
            modpol = modpol.squeeze()

            interpol = modpol.interp(latitude=('z', iotlat),
                                     longitude=('z', iotlon),
                                     method='linear')
            interpdata = np.log(interpol.values)
            iotdiff = iotvals - interpdata
            iotdiff = iotdiff.astype('float32')
            ...
            try:
                ok_val, ok_var = run_kriging(iotlon, iotlat, iotdiff,
                                            kriglon, kriglat,
                                            custom_lags, penalty=1e-6,
                                            device=device, plotting=False,
                                            logscale=logscale)
            except ValueError as e:
                print(e)
                print('Problems with global kriging, skipping this time step')
                continue
            ...
            dist_labsel = distkm > distlab_threshold
            interlabel[dist_labsel] = -1
            interlabel = interlabel.ravel()

            interdata = np.full_like(kriglon, np.nan, dtype='float32')
            intervar = np.full_like(kriglon, np.nan, dtype='float32')
            for i in tqdm(range(0, labs.max()+1), disable=True):
                ...
                tmp_kriglon = kriglon[krigsel]
                tmp_kriglat = kriglat[krigsel]
            ...
            expdata = ok_val.copy()
            expweight = np.exp(-distkm)
            expweight[distkm > distance_threshold] = 0
            expdata[nansel] = expweight[nansel]*interdata[nansel] + (1-expweight[nansel])*ok_val[nansel]

            # add drift correction back to interpolated data
            drift = modpol.interp(latitude=('z', kriglat),
                                  longitude=('z', kriglon),
                                  method='linear')
            logdrift = np.log(drift.values)
            expfinaldata = np.exp(np.log(expdata) + logdrift)

            # now reshape the data
            expkrigdata = np.full(len(gX_flat), np.nan, dtype=np.float32)
            expkrigdata[lsm_sel] = expfinaldata
            ...
            for dataname, data in outdict.items():
                ...
                outds[dataname] = xr.DataArray(tmp,
                                            dims=('lon', 'lat'),
                                            coords={'lat': gridlat, 'lon': gridlon})

            outds['final_kriging'] = xr.DataArray(expkrigdata,
                                                dims=('lon', 'lat'),
                                                coords={'lat': gridlat, 'lon': gridlon})
